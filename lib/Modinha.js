...
function set (source, target, schema, options) {
  Object.keys(schema).forEach(function (key) {
    ...
    if (descriptor.properties) {
      ...
      set(value || {}, target[key], descriptor.properties, options);
    } 
    ...
    // Otherwise treat the key as a simple attribute.
      ...
      // are requested.
      if (!descriptor.private || options.private) {

        // apply a setter instead of directly assigning the value
        if (typeof descriptor.set === 'function') {
          target[key] = descriptor.set(source);
        }

        // set nonwritable property
        else if (descriptor.immutable) {
...
Modinha.prototype.project = function (mapping) {
  ...
  if (typeof mapping === 'string') {
    mapping = Model.mappings[mapping];
  }

  return Model.project(this, {}, mapping);
}
