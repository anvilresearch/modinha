...
Model.create = function (attrs, callback) {
  ...
  async.series({
    ...
    beforeValidate: function (done) {
      async.parallel(validationHooks, function (err, results) {
    ...
    beforeCreate: function (done) {
      async.parallel(creationHooks, function (err, results) {
        if (err) { return done(err); }
        done(null, results);
      });
    },
    ...
    var completionHooks = hooks.complete.map(function (hook) {
      return async.apply(hook, instance, attrs, result);
    });  

    async.parallel(completionHooks, function (err) {
      if (err) { return callback(err); }
...
//   */
//  
//  var createHooks = hooks.create.map(function (hook) {
//    return async.apply(hook, instance, attrs);
//  });
//
//  var validateHooks = hooks.validate.map(function (hook) {
//    return async.apply(hook, instance, attrs);
//  });
//
//  /**
//   * Apply asynchronous "before validation" hooks,
//   * then validate the instance.
//   */
//
//  async.parallel(validateHooks, function (err) {
//    if (err) { return callback(err); }
//
//    var validation = instance.validate();
...
//
//    /**
//     * Apply asynchronous "before create" hooks, 
//     * then create the instance.
//     */
//
//    async.parallel(createHooks, function (err) {
//      if (err) { return callback(err); }
//
//      Constructor.backend.save(instance, function (err) {
//        if (err) { return callback(err); }
